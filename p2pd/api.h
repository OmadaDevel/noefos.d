...
namespace api
{
	...
	// destinations
	std::shared_ptr<p2p::client::ClientDestination> CreateLocalDestination (const p2p::data::PrivateKeys& keys, bool isPublic = true,
		const std::map<std::string, std::string> * params = nullptr);
	std::shared_ptr<p2p::client::ClientDestination> CreateLocalDestination (bool isPublic = false, p2p::data::SigningKeyType sigType = p2p::data::SIGNING_KEY_TYPE_ECDSA_SHA256_P256,
		const std::map<std::string, std::string> * params = nullptr); // transient destinations usually not published
	void DestroyLocalDestination (std::shared_ptr<p2p::client::ClientDestination> dest);
